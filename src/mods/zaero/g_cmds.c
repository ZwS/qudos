...
void Cmd_Give_f (edict_t *ent)
{
	...
	if (deathmatch->value && !sv_cheats->value)
	{
			...
			gi.cprintf 
#endif
		 (ent, PRINT_HIGH, "You must run the server with '+set cheats 1' to enable this command.\n");
		return;
	}
...
qboolean tryUse(edict_t *ent, char *s)
{
	...
	if (!it->use)
	{
			...
			gi.cprintf 
#endif
		 (ent, PRINT_HIGH, "Item is not usable.\n");
		return false;
	}
	index = ITEM_INDEX(it);
...
void altSelect(edict_t *ent, int num)
{
	...
	if (num < 1 || num > 10)
	{
			...
			gi.cprintf 
#endif
		 (ent, PRINT_HIGH, "Invalid weapon index: %i\n", num);
		return;
	}
...
void Cmd_Use_f (edict_t *ent)
{
	...
	if (Q_stricmp(gi.argv(1), "weapon") == 0)
	{
		...
		if (Q_stricmp(s, "") == 0)
			...
			gi.cprintf 
#endif
			(ent, PRINT_HIGH, "weapon index expected (1 to 10)\n");
		else
...
void Cmd_Drop_f (edict_t *ent)
{
	...
	if (!it->drop)
	{
			...
			gi.cprintf 
#endif
		 (ent, PRINT_HIGH, "Item is not dropable.\n");
		return;
	}
	index = ITEM_INDEX(it);
	if (!ent->client->pers.inventory[index])
...
// ACEBOT_END
  ...
  else if(Q_stricmp (cmd, "linesize") == 0)
  {
    ...
    if(ls <= 0.0)
    {
			...
			gi.cprintf 
#endif
       (ent, PRINT_HIGH, "LineSize must be greater than 0\n");
      return;
    }
	...
	else if (Q_stricmp(cmd, "showorigin") == 0)
	{
...
#ifdef WITH_ACEBOT
			safe_cprintf(ent, PRINT_HIGH, "Show origin ON\n");
		else
			safe_cprintf(ent, PRINT_HIGH, "Show origin OFF\n");
#else
      ...
      anim_player_cmd(ent);
		...
		Cmd_Summon_f(ent);
	else if (Q_stricmp(cmd, "runrun") == 0)
		Cmd_Runrun_f(ent);
#endif
