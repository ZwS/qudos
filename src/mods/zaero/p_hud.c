...
#ifdef GAME_MOD
/* <cooplayout/> Internal functions for compute the cooplayout data */
/* ================== */
//
/* ================== */
int
...
/* <cooplayout> CoopScoreboardMessage (cooperative layout) */
/* ================== */
...
CoopScoreboardMessage(edict_t * ent)
{
	...
	for (i = 0; i < game.maxclients; i++) {
		...
		health = Z_MAX(cl_ent->health, 0);
		health = (Z_MIN(health, 100) / 5) * 5;			/* bh0..5..10.. ..100 */
		armor = entArmor(cl_ent);
		armor = Z_MAX(armor, 0);
		armor = (Z_MIN(armor, 100) / 5) * 5;			/* ba0..5..10.. ..100 */
		...
		if (cl_ent == ent) {					/* The player himself */
			Com_sprintf(string + j, sizeof(string) - j,
			    ...
			    x - 35, y + 8, health,
			    y + 13, armor,
			    x - 82, y + 20, va("(%dms)", Z_MIN(cl->ping, 999)));
		} else {
...
==================
*/
void NewDeathmatchScoreboardMessage (edict_t *ent, edict_t *killer)
{
        char    entry[1024];
        char    string[1400];
	int     stringlength = 0;
...
*/
#ifdef GAME_MOD
void HelpComputerMin (edict_t *ent) 
{	
	char	string[1024];
