...
/* NiceAss Start:  */
#define MAX_NET_HISTORY 32
typedef struct net_history_s {
	int		SendsSize[MAX_NET_HISTORY];
	int		SendsTime[MAX_NET_HISTORY];
	int		SendsIndex;
	int		SendsStartTime;

	int		RecsSize[MAX_NET_HISTORY];
	int		RecsTime[MAX_NET_HISTORY];
	int		RecsIndex;
	int		RecsStartTime;
} net_history_t;

extern net_history_t Net_History;
/* :NiceAss End */
 ...
 * directly in C code.
 *
 * The user can access cvars from the console in three ways: r_draworder
 * rints the current value r_draworder 0		sets the current
 * value to 0 set r_draworder 0	as above, but creates the cvar if not present
...
#ifdef HAVE_IPV6
typedef enum {
	NA_LOOPBACK, NA_BROADCAST, NA_IP, NA_IPX, NA_BROADCAST_IPX, NA_IP6, NA_MULTICAST6
} netadrtype_t;
#else
typedef enum {
...
typedef struct {
	...
	int		incoming_reliable_sequence;	/* single bit,
							 * maintained local */

	int		outgoing_sequence;
	int		reliable_sequence;	/* single bit */
	int		last_reliable_sequence;	/* sequence number of last
						 * send */

	/* reliable staging and holding areas */
	sizebuf_t	message;/* writing buffer to send to server */
	byte		message_buf[MAX_MSGLEN - 16];	/* leave space for
							 * header */

	/* message is copied to this buffer when it is first transfered */
	int		reliable_length;
	byte		reliable_buf[MAX_MSGLEN - 16];	/* unacked reliable
							 * message */
} netchan_t;
...
trace_t
CM_BoxTrace(vec3_t start, vec3_t end,
    vec3_t mins, vec3_t maxs,
    int headnode, int brushmask);
trace_t
CM_TransformedBoxTrace(vec3_t start, vec3_t end,
    vec3_t mins, vec3_t maxs,
    int headnode, int brushmask,
...
int
CM_BoxLeafnums(vec3_t mins, vec3_t maxs, int *list,
...
void		CM_SetAreaPortalState(int portalnum, qboolean isopen);
