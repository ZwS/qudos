 ...
 *
 * start end acceleration speed deceleration begin sound end sound target fired
 * when reaching end wait at end
 *
 ...
 * ============== Think_AccelMove
 *
 * The team has completed a frame of movement, so change the speed for the next
 * frame ==============
 */
...
plat_Accelerate(moveinfo_t * moveinfo)
{
	...
	if (moveinfo->current_speed < moveinfo->speed) {
		...
		/*
		 * and cross over the decel_distance; figure the average
		 * speed for the
		 */
		/* entire move */
		p1_distance = moveinfo->remaining_distance - moveinfo->decel_distance;
...
void
Touch_Plat_Center(edict_t * ent, edict_t * other, cplane_t * plane, csurface_t * surf)
{
	if (!other->client)
		return;
	...
	if (ent->moveinfo.state == STATE_BOTTOM)
		...
		ent->nextthink = level.time + 1;	/* the player is still
							 * on the plat, so delay
							 * going down */
}
 ...
 * STOP ANIMATED ANIMATED_FAST You need to have an origin brush as part of
 * this entity.  The center of that brush will be the point around which it
 * is rotated. It will rotate around the Z axis by default.  You can check
 * either the X_AXIS or Y_AXIS box to change that.
 *
 * "speed" determines how fast it moves; default value is 100. "dmg"
 * damage to inflict when blocked (2 default)
 *
 * REVERSE will cause the it to rotate in the opposite direction. STOP mean it
 * will stop moving instead of pushing entities
 */
 ...
 * triggered again.
 *
 * "angle"		determines the opening direction "target"	all entities
 * with a matching targetname will be used "speed"		override the
 * default 40 speed "wait"		override the default 1 second wait
 * (-1 = never return) "lip"		override the default 4 pixel lip
 * remaining at end of move "health"	if set, the button must be killed
 * instead of touched "sounds" 1) silent 2) steam metal 3) wooden clunk 4)
 * metallic click 5) in-out
 */
...
void
button_killed(edict_t * self, edict_t * inflictor, edict_t * attacker, int damage, vec3_t point)
{
 ...
 * DOORS
 *
 * spawn a trigger surrounding the entire team unless it is already targeted by
 * another
 *
 * ======================================================================
 */
 ...
 * TOGGLE ANIMATED_FAST TOGGLE		wait in both the start and end states
 * for a trigger event. START_OPEN	the door to moves to its destination
 * when spawned, and operate in reverse.  It is used to temporarily or
 * permanently close off an area when triggered (not useful for touch or
 * takedamage doors). NOMONSTER	monsters will not trigger this door
 *
 * "message"	is printed when the door is touched if it is a trigger door
 * and it hasn't been fired yet "angle"		determines the opening
 * direction "targetname" if set, no touch field will be spawned and a remote
 * button or trigger field activates the door. "health"	if set, door must be
 * shot open "speed"		movement speed (100 default) "wait"
 * ait before returning (3 default, -1 = never return) "lip"		lip
 * remaining at end of move (8 default) "dmg"		damage to inflict
 * when blocked (2 default) "sounds" 1)	silent 2)	light 3)
 * medium 4)	heavy
 */
...
void
Touch_DoorTrigger(edict_t * self, edict_t * other, cplane_t * plane, csurface_t * surf)
{
	if (other->health <= 0)
		return;
 ...
 * ANIMATED TOGGLE X_AXIS Y_AXIS TOGGLE causes the door to wait in both the
 * start and end states for a trigger event.
 *
 ...
 * area when triggered (not useful for touch or takedamage doors). NOMONSTER
 * monsters will not trigger this door
 *
 * You need to have an origin brush as part of this entity.  The center of that
 * brush will be the point around which it is rotated. It will rotate around
 * the Z axis by default.  You can check either the X_AXIS or Y_AXIS box to
 * change that.
 *
 * "distance" is how many degrees the door will be rotated. "speed" determines
 * how fast the door moves; default value is 100.
 *
 ...
 * button or trigger field activates the door. "health"	if set, door must be
 * shot open "speed"		movement speed (100 default) "wait"
 * ait before returning (3 default, -1 = never return) "dmg"
 *  to inflict when blocked (2 default) "sounds" 1)	silent 2)	light
 * 3)	medium 4)	heavy
 */
 ...
 * ovement speed (25 default) "wait"		wait before returning (-1
 * default, -1 = TOGGLE) "lip"		lip remaining at end of move (0
 * default) "sounds"	(yes, these need to be changed) 0)	no sound 1)
 * water 2)	lava
 */
 ...
 * trigger, it will not begin moving until activated. speed	default 100
 * dmg		default	2 noise	looping sound to play when the train is in
 * motion
 *
 */
...
/*
 * QUAKED func_timer (0.3 0.1 0.6) (-8 -8 -8) (8 8 8) START_ON "wait"
 * ase time between triggering all targets, default is 1 "random"
 * ait variance, default is 0
 *
 * so, the basic time between firing is a random time between (wait - random)
 * and (wait + random)
 *
 * "delay"			delay before first firing when turned on, default is
 * 0
 *
 * "pausetime"		additional delay used only the very first time and
 * only if spawned with START_ON
 *
 ...
 *
 * open_once		doors never closes 1st_left		1st move is
 * left of arrow 1st_down		1st move is down from arrow
 * always_shoot	door is shootebale even if targeted
 *
 * "angle"		determines the direction "dmg"		damage to inflic when
 * blocked (default 2) "wait"		how long to hold in the open position
 * (default 5, -1 means hold)
 */
