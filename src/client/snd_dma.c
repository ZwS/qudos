...
void S_SoundInfo_f(void)
{
	...
	Com_Printf("Samplebits: %d\n", dma.samplebits);
	Com_Printf("Submission_chunk: %d\n", dma.submission_chunk);
	Com_Printf("Speed: %d\n", dma.speed);
}
...
#endif
		    /* load sound driver */
		    snddriver = Cvar_Get("snd_driver", "sdl", CVAR_ARCHIVE);

		    /* Com_Printf("Loading %s sound output driver", snddriver->string); */
		    snprintf(fn, MAX_OSPATH, "%s/snd_%s.so", path, snddriver->string);
		    if (stat(fn, &st) == -1) {
		    	Com_Printf("\nload %s failed: %s\n", fn, strerror(errno));
				return;
		    }
