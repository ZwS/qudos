...
void
ClipCam(vec3_t start, vec3_t end, vec3_t newpos)
{
	int		i;
...
CL_DeltaEntity(frame_t * frame, int newnum, entity_state_t * old, int bits)
{
	...
	if (ent->serverframe != cl.frame.serverframe - 1) {	/* wasn't in last
								 * update, so initialize
								 * some things */
								 
		ent->trailcount = 1024;				/* for diminishing rocket / grenade
						 		 * trails */

		/*
		 * duplicate the current state so lerping doesn't hurt
		 * anything
		 */
		ent->prev = *state;
...
CL_ParsePacketEntities(frame_t * oldframe, frame_t * newframe)
{
	...
	while (1) {
		...
		while (oldnum < newnum) {	/* one or more entities from
						 * the old packet are
						 * unchanged */
		...
		if (bits & U_REMOVE) {	/* the entity present in oldframe is
...
CL_ParseFrame(void)
{
	...
	if (cl.frame.deltaframe <= 0) {
		...
		if (old->serverframe != cl.frame.deltaframe) {	/* The frame that the
								 * server did the delta
								 * from is too old, so we can't 
...
CL_AddPacketEntities(frame_t * frame)
{
	...
	for (pnum = 0; pnum < frame->num_entities; pnum++) {
		...
		if (renderfx & RF_BEAM) {	/* the four beam colors are
		...
		if (cl_models_overbright->value) {
			ent.flags = renderfx | RF_FULLBRIGHT;
		}
		...
		/* if (effects & EF_COLOR_SHELL) */
		if (effects & EF_COLOR_SHELL && (!isclientviewer || (cl_3dcam->value && ((cl_predict->value) && !(cl.frame.playerstate.pmove.pm_flags & PMF_NO_PREDICTION)) && !(cl.attractloop && !(cl.cinematictime > 0 && cls.realtime - cl.cinematictime > 1000))))) {
			/* PMM - at this point, all of the shells have been handled 
			 * if we're in the rogue pack, set up the custom mixing, 
			 * otherwise just keep going */

			/*
		 	* all of the solo colors are fine.  we need to catch any of
		 	* the combinations that look bad
			* (double & half) and turn them into the appropriate color,
			* and make double/quad something special
			*/
			if (renderfx & RF_SHELL_HALF_DAM) {
				if (Developer_searchpath(2) == 2) {

					/*
					 * ditch the half damage shell if any of red,
					 * blue, or double are on
					 */
			...
			if (renderfx & RF_SHELL_DOUBLE) {
				if (Developer_searchpath(2) == 2) {
					...
					/*
					 * lose the yellow shell if we have a red,
					 * blue, or green shell
					 */
					...
					/*
					 * if we have a red shell, turn it to purple
					 * by adding blue
					 */
					if (renderfx & RF_SHELL_RED)
						renderfx |= RF_SHELL_BLUE;
					...
					/*
					 * if we have a blue shell (and not a red
					 * shell), turn it to cyan by adding green
					 */
			 ...
			 */
				...
				if (effects & EF_TRACKER) {	/* lame... problematic? */
					...
					if (cl_blaster_color->value == 1) {
						/* give the bolt a red particle glow */
						CL_BlasterTrailColor(cent->lerp_origin, ent.origin, 235, 50, 50,
						                     1, 0, 0);
						V_AddLight(ent.origin, 200, 1, 0, 0);
					} else if (cl_blaster_color->value == 2) {
						/* give the bolt a green particle glow */
						CL_BlasterTrailColor(cent->lerp_origin, ent.origin, 50, 235, 50,
						                     0, 1, 0);
						V_AddLight(ent.origin, 200, 0, 1, 0);
					} else if (cl_blaster_color->value == 3) {
						/* give the bolt a blue particle glow */
						CL_BlasterTrailColor(cent->lerp_origin, ent.origin, 50, 50, 235,
						                     0, 0, 1);
						V_AddLight (ent.origin, 200, 0, 0, 1);
					} else
#endif
...
CL_AddViewWeapon(player_state_t * ps, player_state_t * ops)
{
	...
	if (weap_shell->value) {
		...
		for (pnum = 0; pnum < cl.frame.num_entities; pnum++) {
			...
			if (s1->effects & (EF_COLOR_SHELL | EF_QUAD | EF_PENT | EF_DOUBLE | EF_HALF_DAMAGE | EF_POWERSCREEN)) {
				gun.flags |= (RF_TRANSLUCENT | s1->renderfx);
				if (s1->effects & EF_POWERSCREEN)
					gun.flags |= RF_SHELL_GREEN;
				if (s1->effects & EF_PENT)
					gun.flags |= RF_SHELL_RED;
				/* V_AddLight(gun.origin, 200, 1.0, 0.5, 0.5); */
				if (s1->effects & EF_QUAD)
					gun.flags |= RF_SHELL_BLUE;
				/* V_AddLight(gun.origin, 200, 0.5, 0.5, 1.0); */
				if (s1->effects & EF_DOUBLE)
					gun.flags |= RF_SHELL_DOUBLE;
				/* V_AddLight(gun.origin, 200, 0.9, 0.7, 0.0); */
				if (s1->effects & EF_HALF_DAMAGE)
					gun.flags |= RF_SHELL_HALF_DAM;
				/* V_AddLight(gun.origin, 200, 0.56, 0.59, 0.45); */
				gun.alpha = 0.1;
				V_AddEntity(&gun);
			}
		}
...
SetUpCamera(void)
{
	if (cl_3dcam->value && ((cl_predict->value) && !(cl.frame.playerstate.pmove.pm_flags & PMF_NO_PREDICTION)) && !(cl.attractloop && !(cl.cinematictime > 0 && cls.realtime - cl.cinematictime > 1000))) {
		...
		if (cl_3dcam_chase->value) {
			...
			/*
			 * move back so looking straight down want make us
			 * look towards ourself
			 */
			{
				vec3_t		temp, temp2;
