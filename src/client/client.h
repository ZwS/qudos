...
#define MAX_CLIENTWEAPONMODELS		20	/* PGM -- upped from 16 to
						 * fit the chainfist vwep */
...
typedef struct {
	...
	/* screen rendering information showing loading plaque between levels or changing rendering dlls*/
	float		disable_screen;
	
	/* if time gets > 30 seconds ahead, break it */
	/* when we receive a frame and cl.servercount */
	...
	/*
	 * demo recording info must be here, so it isn't cleared on level
	 * change
	 */
...
extern cvar_t  *cl_hud_red;
extern cvar_t  *cl_hud_green;
extern cvar_t  *cl_hud_blue;
extern cvar_t  *cl_menu_alpha;
extern cvar_t  *cl_conback_image;
...
typedef struct particle_s {
	...
	void            (*think) (struct particle_s *p, vec3_t org, vec3_t angle, float *alpha, float *size, 
	                          int *image, float *mytime);
	qboolean	thinknext;
#endif
} cparticle_t;
...
/* psychospaz -- client side clipping */
...
#ifdef QMAX
void		V_AddParticle(vec3_t org, vec3_t angle, vec3_t color, float alpha, int alpha_src,
                              int alpha_dst, float size, int image, int flags);
#else
