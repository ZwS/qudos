...
CL_Setenv_f(void)
{
	...
	if (argc > 2) {
		...
		for (i = 2; i < argc; i++) {
			strncat(buffer, Cmd_Argv(i), sizeof(buffer) - strlen(buffer) - 1);
			strncat(buffer, " ", sizeof(buffer) - strlen(buffer) - 1);
		}
...
CL_CheckForResend(void)
{
	...
	if (cls.state == ca_disconnected && Com_ServerState()) {
		...
		/*
		 * cls.connect_time = -99999;	// CL_CheckForResend() will
		 * fire immediately
		 */
	}
 ...
 * ===================== CL_Disconnect
 *
 * Goes from a connected state to full screen console state Sends a disconnect
 * message to the server This is also called on Com_Error, so it shouldn't
 * cause any errors =====================
 */
void
CL_Disconnect(void)
{
	...
	Q_strncpyz((char *)final + 1, "disconnect", sizeof(final));
 ...
 * ================= CL_Snd_Restart_f
 *
 * Restart the sound subsystem so it can pick up new parameters and flush all
 * sounds =================
 */
void
 ...
 * ================= CL_Precache_f
 *
 * The server will send this command right before allowing the client into the
 * server =================
 */
void
CL_Precache_f(void)
...
{
	if (Cmd_Argc() != 2) {
		Com_Printf("usage: loc_add <label>\n");
		return;
	}
...
{
	if (Cmd_Argc() != 2) {
		Com_Printf("usage: loc_save <filename>\n");
		return;
	}
...
CL_InitLocal(void)
{
	...
	cl_explosion_scale = Cvar_Get("cl_explosion_scale", "1", CVAR_ARCHIVE);
	...
	sensitivity = Cvar_Get("sensitivity", "3.5", CVAR_ARCHIVE);
	...
	name = Cvar_Get("name", "Quaker", CVAR_USERINFO | CVAR_ARCHIVE);
	...
	cl_gunalpha = Cvar_Get("cl_gunalpha", "1", CVAR_ARCHIVE);
	cl_drawclock = Cvar_Get("cl_drawclock", "0", CVAR_ARCHIVE);
	cl_drawtimestamps = Cvar_Get("cl_drawtimestamps", "1", CVAR_ARCHIVE);
	cl_hudalpha = Cvar_Get("cl_hudalpha", "1", CVAR_ARCHIVE);
	cl_drawmaptime = Cvar_Get("cl_drawmaptime", "0", CVAR_ARCHIVE);
#ifndef QMAX
	cl_railstyle = Cvar_Get("cl_railstyle", "0", CVAR_ARCHIVE);
	cl_railtrail = Cvar_Get("cl_railtrail", "0", CVAR_ARCHIVE);
	cl_railtrail_color = Cvar_Get("cl_railtrail_color", "0", CVAR_ARCHIVE);
#endif
	cl_sleep = Cvar_Get("cl_sleep", "0", CVAR_ARCHIVE);	/* CPU usage */
	cl_bobbing = Cvar_Get("cl_bobbing", "1", CVAR_ARCHIVE);
	cl_models_overbright = Cvar_Get("cl_models_overbright", "0", CVAR_ARCHIVE);
	cl_underwater_movement = Cvar_Get("cl_underwater_movement", "1", CVAR_ARCHIVE);
	cl_underwater_trans = Cvar_Get("cl_underwater_trans", "0", CVAR_ARCHIVE);
	cl_flashlight = Cvar_Get("cl_flashlight", "1", 0);
	...
	cl_hud_red = Cvar_Get("cl_hud_red", "1", CVAR_ARCHIVE);
	cl_hud_green = Cvar_Get("cl_hud_green", "1", CVAR_ARCHIVE);
	cl_hud_blue = Cvar_Get("cl_hud_blue", "1", CVAR_ARCHIVE);
	cl_menu_alpha = Cvar_Get("cl_menu_alpha", "0", CVAR_ARCHIVE);
	cl_conback_image = Cvar_Get("cl_conback_image", "conback", CVAR_ARCHIVE);
	weap_shell = Cvar_Get("weap_shell", "1", CVAR_ARCHIVE);

	cl_drawlocs = Cvar_Get("cl_drawlocs", "0", 0);

	cl_mouse_cursor = Cvar_Get("cl_mouse_cursor", "0", CVAR_ARCHIVE);
	...
	font_color = Cvar_Get("font_color", "default", CVAR_ARCHIVE);
	
	gl_minimap_x = Cvar_Get("gl_minimap_x", "800", CVAR_ARCHIVE);
	gl_minimap_y = Cvar_Get("gl_minimap_y", "400", CVAR_ARCHIVE);
	...
	/*
	 * Cmd_AddCommand ("packet", CL_Packet_f); // this is dangerous to
	 * leave in
	 */
	...
	/* to work -- all unknown commands are automatically forwarded to the server */
	...
	Cmd_AddCommand("loc_add", CL_AddLoc);
	Cmd_AddCommand("loc_del", CL_DeleteLoc);
	Cmd_AddCommand("loc_save", CL_SaveLoc);
...
CL_WriteConfiguration(void)
{
	...
	Com_sprintf(path, sizeof(path), "%s/QuDos.cfg", FS_Gamedir());
	f = fopen(path, "w");
	if (!f) {
		Com_Printf("Couldn't write QuDos.cfg.\n");
		return;
	}
	fprintf(f, "// generated by QuDos -- http://qudos.quakedev.com\n");
	Key_WriteBindings(f);
	fclose(f);
...
cheatvar_t	cheatvars[] = {
	...
	{"cl_underwater_trans", "0"},
	{"cl_flashlight", "0"},
	{"gl_minimap", "0"},
	{NULL, NULL}
};

cheatvar_t	cheatvars_coop[] = {
	{"timescale", "1"},
	{"timedemo", "0"},
	{"r_drawworld", "1"},
	{"cl_testlights", "0"},
...
CL_FixCvarCheats_Coop(void)
{
	...
	/* find all the cvars if we haven't done it yet */
	if (!numcheatvars) {
		while (cheatvars_coop[numcheatvars].name) {
			cheatvars_coop[numcheatvars].var = Cvar_Get(cheatvars_coop[numcheatvars].name,
			    cheatvars_coop[numcheatvars].value, 0);
			numcheatvars++;
		}
	}
	/* make sure they are all set to the proper values */
	for (i = 0, var = cheatvars_coop; i < numcheatvars; i++, var++) {
		if (strcmp(var->var->string, var->value)) {
			Cvar_Set(var->name, var->value);
		}
	}
 ...
 * =============== CL_Shutdown
 *
 * FIXME: this is a callback from Sys_Quit and Com_Error.  It would be better to
 * run quit through here before the final handoff to the sys code.
 * ===============
 */
void
CL_Shutdown(void)
