...
void            (APIENTRY * qglMultiTexCoord3fvARB) (GLenum target, const GLfloat * v);	/* mpo */
...
static void APIENTRY
logCopyTexImage2D(GLenum target, GLint level, GLenum internalFormat, GLint x, GLint y, GLsizei width, GLsizei height, GLint border)
{
...
static void APIENTRY
logMap2d(GLenum target, GLdouble u1, GLdouble u2, GLint ustride, GLint uorder, GLdouble v1, GLdouble v2, GLint vstride, GLint vorder, const GLdouble * points)
{
	SIG("glMap2d");
...
static void APIENTRY
logMap2f(GLenum target, GLfloat u1, GLfloat u2, GLint ustride, GLint uorder, GLfloat v1, GLfloat v2, GLint vstride, GLint vorder, const GLfloat * points)
{
	SIG("glMap2f");
...
static void APIENTRY
logTexImage1D(GLenum target, GLint level, GLint internalformat, GLsizei width, GLint border, GLenum format, GLenum type, const void *pixels)
{
	SIG("glTexImage1D");
	dllTexImage1D(target, level, internalformat, width, border, format, type, pixels);
}
static void APIENTRY
logTexImage2D(GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const void *pixels)
{
	SIG("glTexImage2D");
...
static void APIENTRY
logTexSubImage2D(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const void *pixels)
{
	SIG("glTexSubImage2D");
 ...
 * appropriate GL stuff.  In Windows this means doing a * LoadLibrary and a
 * bunch of calls to GetProcAddress.  On other * operating systems we need to
...
QGL_Init(const char *dllname)
{
	...
	if ((glw_state.OpenGLLib = dlopen(dllname, RTLD_LAZY)) == 0) {
		...
		/*
		 * ri.Con_Printf(PRINT_ALL, "QGL_Init: Can't load %s from
		 * /etc/ld.so.conf: %s\n",
		 */
		/* dllname, dlerror()); */
		...
		Com_sprintf(fn, sizeof(fn), "%s/%s", path, dllname);
	...
	if ((dlopen("libGLU.so.1", RTLD_LAZY)) == 0) {
		ri.Con_Printf(PRINT_ALL, "%s\n", dlerror());
		return false;
	} else {
	...
	/*
	 * Ben Lane 30 March 2005: Fixed a minor Quake 2 bug whereby the next 2 
	 * qgl function pointers were loaded with invalid addresses. Original
	 * code attempted to initialise them with GPA( "glGetLevelParameterfv" )
	 * and GPA( "glGetLevelParameteriv" ) crucially missing the "Tex". 
	 */
